...
def createDb(cursor):
    cursor.execute('CREATE TABLE IF NOT EXISTS problem (name TEXT PRIMARY KEY ON CONFLICT IGNORE, type TEXT NOT NULL CONSTRAINT problem_type_constraint CHECK (type IN ("MIN", "MAX", "SAT"))) WITHOUT ROWID')
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS result (year INT NOT NULL, solver TEXT REFERENCES solver NOT NULL, problem TEXT REFERENCES problem NOT NULL, instance TEXT NOT NULL, solved INT NOT NULL CONSTRAINT result_solved_constraint CHECK (solved in (0, 1)), complete INT NOT NULL CONSTRAINT result_complete_constraint CHECK (complete in (0, 1)), time INT, objective_value INT, CONSTRAINT result_unique_constraint UNIQUE (year, solver, problem, instance) ON CONFLICT IGNORE)')
    cursor.execute('CREATE INDEX IF NOT EXISTS solver_index ON solver(name)')
    cursor.execute('CREATE INDEX IF NOT EXISTS result_year_index ON result(year)')
    cursor.execute('CREATE INDEX IF NOT EXISTS result_solver_index ON result(solver)')
    cursor.execute('CREATE INDEX IF NOT EXISTS result_index ON result(year, solver, problem, instance)')
    ...
    for (solver, states, times, objectives) in zip(solvers, results['results'], results['times'], results['objectives']):
         cursor.executemany(
             ...
             zip(repeat(year, len(jobs)),
                 ...
                 map(lambda state: int(stateDict[state][0]), states), # solved
                 map(lambda state: int(stateDict[state][1]), states), # complete
                 map(lambda time: None if time == ' ' else int(time), times),
                 map(lambda objective: None if objective == ' ' else int(objective), objectives)))
